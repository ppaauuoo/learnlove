-- systems/boss_ai.lua: boss state machine system
local Concord = require("deps.concord")
local Combat = require("combat")

local BossAISystem = Concord.system({
    bosses = {"bossAI", "position", "velocity", "physics", "health", "knockback"},
    players = {"playerInput", "position"},
})
...
local function beginAttack(b, pos, vel, phys, ppos)
    ...
    elseif b.attackType == "dash" then
        if b.phase == 3 then
            b.stateTimer = 0.8
            vel.vx = (ppos.x < pos.x and -1 or 1) * 1000
        else
            b.stateTimer = 0.25
            vel.vx = (ppos.x < pos.x and -1 or 1) * 600
        end
...
local function updateAttack(b, pos, vel, ppos)
    if b.attackType == "slam" then
...
function BossAISystem:update(dt)
    -- Get player position (first player entity)
    ...
    for _, e in ipairs(self.bosses) do
        ...
        if b.state ~= "attack" and kb.timer > 0 then
